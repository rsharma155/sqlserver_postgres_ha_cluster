...
DECLARE 
    v_id INT;
    v_final_sku TEXT := p_sku;
BEGIN
    IF EXISTS (SELECT 1 FROM products WHERE sku = p_sku) THEN
        v_final_sku := p_sku || '-' || LPAD(FLOOR(RANDOM() * 1000000)::INT::TEXT, 6, '0');
    END IF;
    
    INSERT INTO products (sku, name, description, unit_price, cost_price, is_active)
    VALUES (v_final_sku, p_name, 'Generated by load test', p_price, p_cost, TRUE)
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM projects WHERE code = v_code) THEN
        v_code := v_code || '-' || LPAD(FLOOR(RANDOM() * 1000)::INT::TEXT, 3, '0');
    END IF;
